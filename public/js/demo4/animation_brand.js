document.addEventListener("DOMContentLoaded", async () => {
    ...
    const squareUrl = `${root}svg/Principal_Brand.svg`.replace(
        /([^:])\/\//g,
        "$1/",
    );
    const rectUrl = `${root}svg/Terciary_Brand.svg`.replace(
        /([^:])\/\//g,
        "$1/",
    );

    console.log("🔍 Fetching SVGs:", { squareUrl, rectUrl });

    const [squareText, rectText] = await Promise.all([
        fetch(squareUrl).then((r) => {
            if (!r.ok) throw new Error(`404: ${squareUrl}`);
            return r.text();
        }),
        fetch(rectUrl).then((r) => {
            if (!r.ok) throw new Error(`404: ${rectUrl}`);
            return r.text();
        }),
    ]).catch((err) => {
        console.error("❌ Error loading branding SVGs:", err);
        return [null, null];
    });

    if (!squareText || !rectText) return;

    const squareContainer = document.getElementById("svg-square");
    const rectContainer = document.getElementById("svg-rect");
    if (!squareContainer || !rectContainer) return;
    ...
    function setInitialState(context, squareGroup, rectGroup) {
        const activeKey = Object.keys(endpoints).find(
            (key) => context.conditions[key],
        );
