(() => {
    ...
    if (!video || !slider || !hourLabel) return;

    // Ruta del video
    const root = window.rootPath || "/";
    video.src = `${root}video/trafico_cdmx.mp4`;
    ...
    video.addEventListener("timeupdate", () => {
        const currentHour =
            Math.floor(video.currentTime / SECONDS_PER_HOUR) % 24;
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        video.play().catch((err) => {
            console.warn(
                "Autoplay bloqueado, requiere interacción del usuario:",
                err,
            );
        });
    });
